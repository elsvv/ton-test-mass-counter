...
function formSendMsgAction(msg: MessageToSend, mode: number): Slice {
    let b = beginCell().storeUint(0x18, 6).storeAddress(msg.recipient).storeCoins(msg.value).storeUint(0, 105);
    if (msg.init) {
        b.storeUint(3, 2);
    ...
    return beginCell().storeUint(0x0ec3c86d, 32).storeUint(mode, 8).storeRef(b.endCell()).endCell().asSlice();
}

function formSetCodeAction(code: Cell): Slice {
    return beginCell().storeUint(0xad4de08e, 32).storeRef(code).endCell().asSlice();
}

export class PWallet implements Contract {
    constructor(readonly address: Address, readonly init?: { code: Cell; data: Cell }) {}

    static createFromAddress(address: Address) {
        return new PWallet(address);
    }

    static createFromPublicKey(publicKey: Buffer, workchain = 0) {
        const data = beginCell().storeBuffer(publicKey, 32).storeUint(0, 16).endCell();
        const init = { code: walletCode, data };
        return new PWallet(contractAddress(workchain, init), init);
    }
    ...
    private async sendActions(provider: ContractProvider, actions: Slice[], keypair: KeyPair) {
        let actionsCell = Cell.EMPTY;
        for (const action of actions) {
            actionsCell = beginCell().storeRef(actionsCell).storeSlice(action).endCell();
        }
        ...
        await provider.external(beginCell().storeBuffer(signature, 64).storeRef(msgInner).endCell());
    }

    async sendTransfers(provider: ContractProvider, keypair: KeyPair, messages: MessageToSend[]) {
    ...
    async sendSetCode(provider: ContractProvider, keypair: KeyPair, code: Cell) {
